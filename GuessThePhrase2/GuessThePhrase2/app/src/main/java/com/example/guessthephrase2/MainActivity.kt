                    ...
                    when(etUserGusess.hint){
                        "Guess the letter" -> {
                            if(UserGuess.length-1 > 1){
                                Snackbar.make(clMain,"Allow enter just the guess of Letter",Snackbar.LENGTH_SHORT).show()
                            }else if(!(Available(UserGuess[0]))){
                                Snackbar.make(clMain,"You Guess this letter before, Try again",Snackbar.LENGTH_SHORT).show()
                            }else{
