...
class DBHlpr(context: Context):SQLiteOpenHelper(context,"details.db",null,1){
    ...
    fun getData(): ArrayList<CelebrityModel> {
        ...
        if(c.moveToFirst()){
            do {
                ...
                arr.add(CelebrityModel(ID,Name,Taboo1, Taboo2, Taboo3))
            }while (c.moveToNext())
        }
        DB.close()
        return arr
    ...
    fun deleteData(SelectCelebrity: CelebrityModel): Int {
        val db = this.writableDatabase
        val success = db.delete("Celebrity", "$ID_key ="+SelectCelebrity.Id,null)
        db.close()
        return success
    }
    fun updateDate(new: CelebrityModel): Int {
        val db = this.writableDatabase
        val cv = ContentValues()
        cv.put("Name",new.Name)
        cv.put("Taboo1",new.Taboo1)
        cv.put("Taboo2",new.Taboo2)
        cv.put("Taboo3",new.Taboo3)
        val success = db.update("Celebrity", cv,"id ="+new.Id,null)
        db.close()
        return success
    }
}